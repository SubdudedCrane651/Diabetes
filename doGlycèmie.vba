Sub doGlycèmie()
    ...
    For b = 5 To 1000
        count = count + 1
        breakfast = ActiveWorkbook.Sheets("Diabetes_Control").Cells(b, 1)
        If ActiveWorkbook.Sheets("Diabetes_Control").Cells(b - 1, 1) <> Empty And Beforelunch And ActiveWorkbook.Sheets("Diabetes_Control").Cells(b - 1, 1) = ActiveWorkbook.Sheets("Diabetes_Control").Cells(b, 1) Then
            samedate = True
            ActiveWorkbook.Sheets("Glycèmie_De_Richard_Perreault").Cells(b - 1, 1) = breakfast
        Else
            ActiveWorkbook.Sheets("Glycèmie_De_Richard_Perreault").Cells(b, 1) = breakfast
        End If
        breakfastglycèmie = ActiveWorkbook.Sheets("Diabetes_Control").Cells(b, 3)
        Time = ActiveWorkbook.Sheets("Diabetes_Control").Cells(b, 2)
        If Time > "1:00:00" And Time <= "9:00:00" Then
            If samedate Then
                ActiveWorkbook.Sheets("Glycèmie_De_Richard_Perreault").Cells(b - 1, 2) = breakfastglycèmie
            Else
                ActiveWorkbook.Sheets("Glycèmie_De_Richard_Perreault").Cells(b, 2) = breakfastglycèmie
            End If
            Beforelunch = False
            samedate = False
        Else
            ActiveWorkbook.Sheets("Glycèmie_De_Richard_Perreault").Cells(b, 4) = breakfastglycèmie
            Beforelunch = True
        End If
        If breakfast = Empty Then
            Exit For
        End If
    Next
    ...
    While dodo <> Empty
        ...
        If dodo = dodo2 Then
            ...
            dinerglyc èmie = ActiveWorkbook.Sheets("Diabetes_Control").Cells(5 + count4, 11)
